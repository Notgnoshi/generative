...
{
    const auto geometry = generative::io::from_wkt("LINESTRING(0 0, 1 1, 2 2)");
...
{
    const auto geometry = generative::io::from_wkt("LINESTRING Z (0 0 1, 0 0 2, 0 0 3)");
    ASSERT_TRUE(geometry);
    ASSERT_EQ(geometry->getNumPoints(), 3);
    ASSERT_EQ(geometry->getCoordinateDimension(), 3);
...
{
    const auto geometry = generative::io::from_wkt("POLYGON((0 0, 1 1, 2 2, 0 0))");
    ASSERT_TRUE(geometry);
    const auto coords = geometry->getCoordinates();
    ASSERT_TRUE(coords);
...
{
    const auto geometry = generative::io::from_wkt("POINT(0 0)");
    ASSERT_TRUE(geometry);
